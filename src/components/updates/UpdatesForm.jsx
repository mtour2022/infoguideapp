...
import UpdateFormDataModel from "../../datamodel/updates_model"; 
...
export default function UpdateFormData() {
    const [updatesFormData, setUpdatesFormData] = useState(new UpdateFormDataModel());
    ...
    const handleSubmit = async (e) => {
        ...
        Swal.fire({
            title: 'Submitting...',
            text: 'Please wait while we submit your update.',
        ...
        try {
            ...
            let headerImageURL = await uploadImageToFirebase(
                ...
                `updates/${Date.now()}_${updatesFormData.headerImage}`
            );
            ...
            const bodyImagesURLs = await Promise.all(
                updatesFormData.body.map(async (section, index) => {
                    if (section.image) {
                        const imageURL = await uploadImageToFirebase(section.image, `updates/${Date.now()}_${section.image.name}`);
                        return imageURL;
                    }
                    return "";
                })
            ...
            const update = {
                id: "",
                dateTimeStart: updatesFormData.dateTimeStart,
                dateTimeEnd: updatesFormData.dateTimeEnd,
                category: updatesFormData.category,
                title: updatesFormData.title,
                headerImage: headerImageURL,
                headerImageSource: updatesFormData.headerImageSource,
                body: updatesFormData.body.map((section, index) => ({
                    subtitle: section.subtitle,
                    body: section.body,
                    image: bodyImagesURLs[index] || "",
                    image_source: section.image_source, 
                })),
                tags: updatesFormData.tags,
                references: updatesFormData.references,
                socials: updatesFormData.socials,
                origin: updatesFormData.origin,
            };

            const docRef = await addDoc(collection(db, "updates"), update);
            const updatesDoc = doc(db, "updates", docRef.id);
            await updateDoc(updatesDoc, { id: docRef.id });
            ...
            Swal.fire({
                title: "Error",
                text: "There was an issue submitting your update. Please try again.",
                icon: "error",
            });
        }
    };
              ...
              <Form className="custom-form body-container"  onSubmit={handleSubmit}  onKeyDown={(e) => e.key === "Enter" && e.preventDefault()}>
                <Row>
                    <Col md={6}>
                        <Form.Group controlId="category" className="mb-3">
                                    ...
                                    <Form.Select
                                      name="category"
                                      value={updatesFormData.category}
                                      onChange={handleChange}
                                    >
                                      <option value="" disabled>Select Category</option>
                                      {updatesCategoryOptions.map((option, index) => (
                ...
                <Row>
                    ...
                    <Form.Group className="mb-3">
                        ...
                        <Form.Control
                            type="datetime-local"
                            name="dateTimeStart"
                            value={updatesFormData.dateTimeStart}
                            onChange={handleChange}
                            required
                        />
                    </Form.Group>
                    ...
                    <Form.Group className="mb-3">
                        ...
                        <Form.Control
                            type="datetime-local"
                            name="dateTimeEnd"
                            value={updatesFormData.dateTimeEnd}
                            onChange={handleChange}
                            min={updatesFormData.dateTimeStart} // Prevents selecting an earlier date
                            disabled={!updatesFormData.dateTimeStart} // Disable if Start Date is empty
                            required
                        />
                        </Form.Group>
                    </Col>
                ...
                <Row>
                    ...
                    <Form.Group className="my-2">
                        ...
                        <HeaderImageDropzone
                        storyForm={updatesFormData}
                        setStoryForm={setUpdatesFormData}
                ...
                <Row>
                    ...
                    <TextGroupInputField
                        ...
                        editingItems={updatesFormData.headerImageSource}
                        resetKey={resetKey} 
                        />
                    </Col>
                </Row>
               ...
               <Row>
                <Col md={12}>
                    ...
                    {updatesFormData.body.map((section, index) => (
                ...
                <Row>
                    ...
                    <TextGroupInputField
                        ...
                        editingItems={updatesFormData.origin}
                        resetKey={resetKey} 
                        caption="the office where the update originated or referring to"
                        />
                    </Col>
                </Row>
                <Row>
                    ...
                    <TextGroupInputField
                        ...
                        editingItems={updatesFormData.references}
                        resetKey={resetKey} 
                        />
                    </Col>
                </Row>
                ...
                <Row>
                    ...
                    <TextGroupInputField
                        ...
                        editingItems={updatesFormData.socials}
                        resetKey={resetKey} 
                        />
                    </Col>
                </Row>
                <Row>
                    ...
                    <TextGroupInputField
                        ...
                        editingItems={updatesFormData.tags}
                        resetKey={resetKey} 
