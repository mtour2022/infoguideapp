...
export default function VirtualTourGuideForm({}) {
        ...
        try {
            ...
            const bodyImagesURLs = await Promise.all(
                virtualTourGuideFormData.body.map(async (section, index) => {
                    if (section.image) {
                        const imageURL = await uploadImageToFirebase(section.image, `virtualTourGuides/${Date.now()}_${section.image.name}`);
                        return imageURL;
                    }
                    return "";
                })
            ...
            // Create an instance of VirtualTourGuideFormData and populate it
            const virtualTourGuideData = new VirtualTourGuideFormData();
            virtualTourGuideData.id = ""; // Firestore will generate this
            virtualTourGuideData.title = virtualTourGuideFormData.title;
            virtualTourGuideData.body = virtualTourGuideFormData.body.map((section, index) => ({
              subtitle: section.subtitle || "",
              body: section.body || "",
              image: bodyImagesURLs[index] || "",
              image_source: section.image_source || "",
            }));        
            virtualTourGuideData.references = virtualTourGuideFormData.references || [];;
    
            // Save VirtualTourGuideFormData data to Firestore
            const docRef = await addDoc(collection(db, "virtualTourGuides"), virtualTourGuideData.toJSON());
            const virtualTourGuideDoc = doc(db, "virtualTourGuides", docRef.id);
            await updateDoc(virtualTourGuideDoc, { id: docRef.id });
    
            Swal.fire({
                title: "Virtual Tour Guide Submitted",
                text: "Your Virtual Tour Guide has been submitted successfully!",
                icon: "success",
            });
