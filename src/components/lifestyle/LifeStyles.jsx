...
export default function LifeStylesForm({}) {
  ...
  // Upload image to Firebase Storage (only when submit is clicked)
    ...
    const handleSubmit = async (e) => {
        ...
        Swal.fire({
            title: "Submitting...",
            text: "Please wait while we submit your lifeStyle data.",
        ...
        try {
            ...
            let headerImageURL = lifestylesFormData.headerImage
                ? await uploadImageToFirebase(lifestylesFormData.headerImage, `lifeStyles/${Date.now()}_${lifestylesFormData.headerImage.name}`)
                : null;
            ...
            const bodyImagesURLs = await Promise.all(
                lifestylesFormData.body.map(async (section, index) => {
                    if (section.image) {
                        const imageURL = await uploadImageToFirebase(section.image, `lifeStyles/${Date.now()}_${section.image.name}`);
                        return imageURL;
                    }
                    return "";
                })
            );
            // Create an instance of lifestylesFormData and populate it
            const lifeStyleData = new LifeStyleFormData();
            lifeStyleData.id = ""; // Firestore will generate this
            lifeStyleData.title = lifestylesFormData.title;
            lifeStyleData.body = lifestylesFormData.body.map((section, index) => ({
              subtitle: section.subtitle || "",
              body: section.body || "",
              image: bodyImagesURLs[index] || "",
              image_source: section.image_source || "",
            })); 
            lifeStyleData.headerImage = headerImageURL;
            lifeStyleData.references = lifestylesFormData.references || [];;
    
            // Save lifestylesFormData data to Firestore
            const docRef = await addDoc(collection(db, "lifeStyles"), lifeStyleData.toJSON());
            const lifeStyleDoc = doc(db, "lifeStyles", docRef.id);
            await updateDoc(lifeStyleDoc, { id: docRef.id });
    
            Swal.fire({
                title: "lifeStyle Submitted",
                text: "Your lifeStyle has been submitted successfully!",
                icon: "success",
            });
            ...
            console.error("Error submitting lifeStyle:", error);
            Swal.fire({
                title: "Error",
                text: "There was an issue submitting your lifeStyle. Please try again.",
                icon: "error",
            });
        }
    };
