...
export default function EditTouristFAQForm({editingItem, toAddForm}) {
          ...
          touristFAQFormData.body.map(async (section, index) => {
            if (section.image instanceof File) {
              ...
              return await uploadImageToFirebase(
                section.image,
                `touristFAQs/${Date.now()}_${section.image.name}`
              );
            }
           ...
           const touristFAQData = {
            id: editingItem ? editingItem.id : "", // Use existing ID for updates
            title: touristFAQFormData.title,
            body: touristFAQFormData.body.map((section, index) => ({
              subtitle: section.subtitle,
              body: section.body,
              image: bodyImagesURLs[index] || "",
              image_source: section.image_source,
            })),            
            references: touristFAQFormData.references || [],
          };
           ...
           const touristFAQDocRef = doc(db, "touristFAQs", touristFAQFormData.id);
             try {
                           await updateDoc(touristFAQDocRef, touristFAQData);
                           Swal.fire({
                             title: "touristFAQ Updated",
                             text: "Your touristFAQ has been updated successfully!",
                             icon: "success",
                           });
                           resetForm();
                           toAddForm();
                           ...
                           Swal.fire({
                             title: "Update Failed",
                             text: "There was an error updating the touristFAQ.",
                             icon: "error",
                           });
                         }
