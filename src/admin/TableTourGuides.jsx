...
import TourGuideForm from "../components/tourguides/TourGuidesForm";
import EditingAccommodationForm from "../components/accommodations/EditAccommodatonForm";
import Swal from "sweetalert2";
import EditingTourGuideForm from "../components/tourguides/EditTourGuidesForm";
...
const TourGuideTable = () => {
  ...
  const handleDelete = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        await deleteDoc(doc(db, "tourguides", id));
  ...
  return (
    <Container>
      <h1 className="text-2xl font-bold mb-1">Tour Guide</h1>
      <p>List of Department of Tourism (DOT) Accredited Tour Guides In Malay</p>
      ...
      <Row>
        <Col md={6} className="mb-1 me-2 col">
          <Form.Control
            type="text"
            placeholder={`Search up to over ${data.length} tourguides`}
            value={search}
      ...
      {editingItem ? (
        <EditingTourGuideForm
        editingItem={editingItem}
        toAddForm={() => setEditingItem(null)}
        ></EditingTourGuideForm>
      ) : (
