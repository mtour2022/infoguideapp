...
import IncomingEventsForm from "../components/incomingEvents/IncomingEventsForm";
import EditIncomingEventsForm from "../components/incomingEvents/EditIncomingEventsForm";

const IncomingEventTable = () => {
  ...
  const fetchData = async () => {
      try {
      const querySnapshot = await getDocs(collection(db, "incomingEvents"));
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this entry?")) {
      await deleteDoc(doc(db, "incomingEvents", id));
  ...
  // Export to Excel
   const exportToExcel = () => {
      ...
      XLSX.writeFile(wb, `tourism_incomingevents_infoguide_${getFormattedDate()}.xlsx`);
    };
  ...
  return (
    <Container>
      <h1 className="text-2xl font-bold mb-1">Incoming Events</h1>
      <p>List of Incoming Events.</p>
...
export default IncomingEventTable;
