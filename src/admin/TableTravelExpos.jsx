...
import TravelExposForm from "../components/travelExpos/TravelExpo";
import EditTravelExposForm from "../components/travelExpos/EditTravelExpos";

const TravelExposTable = () => {
  ...
  const [loading, setLoading] = useState(true);
    ...
    const fetchData = async () => {
      try {
      const querySnapshot = await getDocs(collection(db, "travelExpos"));
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this entry?")) {
      await deleteDoc(doc(db, "travelExpos", id));
  ...
  // Export to Excel
   const exportToExcel = () => {
      ...
      XLSX.writeFile(wb, `tourism_travelExpos_infoguide_${getFormattedDate()}.xlsx`);
    };
  ...
  return (
    <Container>
      <h1 className="text-2xl font-bold mb-1">Travel Expos, Exhibits, Summits, Fairs, Conferences, Conventions, B2B and Roadshows</h1>
      <p>List of Travel Expos.</p>
      ...
      <Row className="mb-4 mt-4">
        <Col md={4} className="mb-2 me-2 col">
          <Form.Control
            type="text"
            placeholder={`Search up to over ${data.length} Travel Expos`}
            value={search}
...
export default TravelExposTable;
