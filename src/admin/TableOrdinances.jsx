...
import OrdinancesForm from "../components/ordinances/Ordinances";
import EditOrdinancesForm from "../components/ordinances/EditOrdinances";

const OrdinanceTable = () => {
  ...
  const [loading, setLoading] = useState(true);
    ...
    const fetchData = async () => {
      try {
      const querySnapshot = await getDocs(collection(db, "ordinances"));
  ...
  const filteredData = data.filter(
    (item) =>
      ...
      (
        ...
        item.ordinance.toLowerCase().includes(search.toLowerCase()) ||
  ...
  // Export to Excel
   const exportToExcel = () => {
      ...
      XLSX.writeFile(wb, `tourism_ordinances_infoguide_${getFormattedDate()}.xlsx`);
    };
  ...
  return (
    <Container>
      <h1 className="text-2xl font-bold mb-1">Beach Laws</h1>
      <p>List of Municipal Ordinances pertaining to beach regulations.</p>
      ...
      <Row className="mb-4 mt-4">
        <Col md={4} className="mb-2 me-2 col">
          <Form.Control
            type="text"
            placeholder={`Search up to over ${data.length} ordinances`}
            value={search}
