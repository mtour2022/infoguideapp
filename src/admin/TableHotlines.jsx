...
import HotlinesForm from "../components/hotlines/Hotlines";
import EditHotlinesForm from "../components/hotlines/EditHotelines";
...
const HotlineTable = () => {
  ...
  const [loading, setLoading] = useState(true);
    ...
    const fetchData = async () => {
      try {
      const querySnapshot = await getDocs(collection(db, "hotlines"));
  ...
  const filteredData = data.filter(
    (item) =>
      ...
      (
        ...
        item.landline.toLowerCase().includes(search.toLowerCase()) ||
        item.satellite.toLowerCase().includes(search.toLowerCase()) ||
  ...
  // Export to Excel
   const exportToExcel = () => {
      const flattenedData = data.map(({ name, ...rest }) => {
        return {
          ...rest,
          mobile: Array.isArray(rest.mobile) ? rest.mobile.join(", ") : "", // Use rest.facilities
          landline: Array.isArray(rest.landline) ? rest.landline.join(", ") : "",
          satellite: Array.isArray(rest.satellite) ? rest.satellite.join(", ") : "",
      ...
      XLSX.writeFile(wb, `tourism_hotlines_infoguide_${getFormattedDate()}.xlsx`);
    };
  ...
  return (
    <Container>
      <h1 className="text-2xl font-bold mb-1">Tourist Hotlines</h1>
      <p>List of Tourist Hotlines.</p>
      ...
      <Row className="mb-4 mt-4">
        <Col md={4} className="mb-2 me-2 col">
          <Form.Control
            type="text"
            placeholder={`Search up to over ${data.length} hotlines`}
            value={search}
