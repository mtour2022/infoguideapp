...
import TourismProjectsForm from "../components/tourismProjects/TourismProject";
import EditTourismProjectsForm from "../components/tourismProjects/EditTourismProject";

const TourismProjectsTable = () => {
  ...
  const [loading, setLoading] = useState(true);
    ...
    const fetchData = async () => {
      try {
      const querySnapshot = await getDocs(collection(db, "tourismProjects"));
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this entry?")) {
      await deleteDoc(doc(db, "tourismProjects", id));
  ...
  // Export to Excel
   const exportToExcel = () => {
      ...
      XLSX.writeFile(wb, `tourism_tourismProjects_infoguide_${getFormattedDate()}.xlsx`);
    };
  ...
  return (
    <Container>
      <h1 className="text-2xl font-bold mb-1">Tourism Projects</h1>
      <p>List of Tourism Projects.</p>
      ...
      <Row className="mb-4 mt-4">
        <Col md={4} className="mb-2 me-2 col">
          <Form.Control
            type="text"
            placeholder={`Search up to over ${data.length} Tourism Projects`}
            value={search}
...
export default TourismProjectsTable;
