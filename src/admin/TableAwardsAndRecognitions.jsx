...
import AwardsAndRecognitionsForm from "../components/awardsAndRecognitions/AwardsAndRecognitions";
import EditAwardsAndRecognitionsForm from "../components/awardsAndRecognitions/EditAwardsAndRecognitions";

const AwardsAndRecognitionsTable = () => {
  ...
  const [loading, setLoading] = useState(true);
    ...
    const fetchData = async () => {
      try {
      const querySnapshot = await getDocs(collection(db, "awardsAndRecognitions"));
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this entry?")) {
      await deleteDoc(doc(db, "awardsAndRecognitions", id));
  ...
  // Export to Excel
   const exportToExcel = () => {
      ...
      XLSX.writeFile(wb, `tourism_awardsAndRecognitions_infoguide_${getFormattedDate()}.xlsx`);
    };
  ...
  return (
    <Container>
      <h1 className="text-2xl font-bold mb-1">Tourism Awards and Recognitions</h1>
      ...
      <Row className="mb-4 mt-4">
        <Col md={4} className="mb-2 me-2 col">
          <Form.Control
            type="text"
            placeholder={`Search up to over ${data.length} Awards And Recognitionss`}
            value={search}
...
export default AwardsAndRecognitionsTable;
