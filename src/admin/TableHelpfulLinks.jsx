...
import HelpfulLinksForm from "../components/helpfulLinks/HelpfulLinks";
import EditHelpfulLinksForm from "../components/helpfulLinks/EditHelpfulLinks";

const HelpfulLinksTable = () => {
  ...
  const [loading, setLoading] = useState(true);
    ...
    const fetchData = async () => {
      try {
      const querySnapshot = await getDocs(collection(db, "helpfulLinks"));
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this entry?")) {
      await deleteDoc(doc(db, "helpfulLinks", id));
  ...
  // Export to Excel
   const exportToExcel = () => {
      ...
      XLSX.writeFile(wb, `tourism_helpfulLinks_infoguide_${getFormattedDate()}.xlsx`);
    };
  ...
  return (
    <Container>
      <h1 className="text-2xl font-bold mb-1">Tourist Links</h1>
      <p>List of Helpful Links for tourists.</p>
      ...
      <Row className="mb-4 mt-4">
        <Col md={4} className="mb-2 me-2 col">
          <Form.Control
            type="text"
            placeholder={`Search up to over ${data.length} helpful links`}
            value={search}
