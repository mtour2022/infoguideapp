...
import VirtualTourGuideForm from "../components/virtualTourGuide/VirtualTourGuide";
import EditVirtualTourGuideForm from "../components/virtualTourGuide/EditVirtualTourGuide";

const VirtualTourGuideTable = () => {
  ...
  const [loading, setLoading] = useState(true);
    ...
    const fetchData = async () => {
      try {
      const querySnapshot = await getDocs(collection(db, "virtualTourGuides"));
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this entry?")) {
      await deleteDoc(doc(db, "virtualTourGuides", id));
  ...
  // Export to Excel
   const exportToExcel = () => {
      ...
      XLSX.writeFile(wb, `tourism_virtualTourGuides_infoguide_${getFormattedDate()}.xlsx`);
    };
  ...
  return (
    <Container>
      <h1 className="text-2xl font-bold mb-1">Virtual Tour Guides</h1>
      <p>List of Virtual Tour Guide and Marketing and Promotions.</p>
      ...
      <Row className="mb-4 mt-4">
        <Col md={4} className="mb-2 me-2 col">
          <Form.Control
            type="text"
            placeholder={`Search up to over ${data.length} Virtual Tour Guides`}
            value={search}
...
export default VirtualTourGuideTable;
