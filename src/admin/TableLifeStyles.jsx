...
import EditLifeStylesForm from "../components/lifestyle/EditLifeStyles";
import LifeStylesForm from "../components/lifestyle/LifeStyles";
...
const LifeStyleTable = () => {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this entry?")) {
      await deleteDoc(doc(db, "lifeStyles", id));
  ...
  // Export to Excel
   const exportToExcel = () => {
      ...
      XLSX.writeFile(wb, `tourism_lifeStyles_infoguide_${getFormattedDate()}.xlsx`);
    };
  ...
  return (
    <Container>
      <h1 className="text-2xl font-bold mb-1">Life Style </h1>
      <p>List Life Styles of Locals.</p>
      ...
      <Row className="mb-4 mt-4">
        <Col md={4} className="mb-2 me-2 col">
          <Form.Control
            type="text"
            placeholder={`Search up to over ${data.length} lifeStyles`}
            value={search}
