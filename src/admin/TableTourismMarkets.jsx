...
import EditTourismMarketsForm from "../components/tourismMarkets/EditTourismMarket";

const TourismMarketsTable = () => {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this entry?")) {
      await deleteDoc(doc(db, "tourismMarkets", id));
  ...
  // Export to Excel
   const exportToExcel = () => {
      ...
      XLSX.writeFile(wb, `tourism_tourismMarkets_infoguide_${getFormattedDate()}.xlsx`);
    };
  ...
  return (
    <Container>
      <h1 className="text-2xl font-bold mb-1">Tourism Markets</h1>
      <p>List of Tourism Markets.</p>
      ...
      <Row className="mb-4 mt-4">
        <Col md={4} className="mb-2 me-2 col">
          <Form.Control
            type="text"
            placeholder={`Search up to over ${data.length} Tourism Markets`}
            value={search}
...
export default TourismMarketsTable;
